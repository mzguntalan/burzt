...
makeCurve [] t = (0.0, 0.0) :: Point
makeCurve [p] t = p
makeCurve [start, end] t = start VectorGraphic.+ (t VectorGraphic.* (end VectorGraphic.- start))
makeCurve [start, mid, end] t =
    (a VectorGraphic.* start) VectorGraphic.+ (b VectorGraphic.* mid) VectorGraphic.+ (c VectorGraphic.* end)
  where
...
binarySearchIncreasing incFunc numDecimalPlaces start end = binarySearchIncreasing' incFunc numDecimalPlaces start end 0
  where
    binarySearchIncreasing' incFunc numDecimalPlaces start end depth
        | depth == 10 = mid
